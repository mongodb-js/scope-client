/**
 * A `stream.Writable` wrapper around an unordered bulk insert.
 *
 * @api private
 */
var Writable = require('stream').Writable;
var util = require('util');
var debug = require('debug')('mongodb-scope-client:bulk');

function BulkWriteStream(collection, opts) {
  if (!(this instanceof BulkWriteStream)) {
    return new BulkWriteStream(collection, opts);
  }
  ...
  BulkWriteStream.super_.call(this, {
    objectMode: true
  });
}
...
BulkWriteStream.prototype._write = function(doc, enc, next) {
  if (!this.ready) {
    return this.on('writeable', this._write.bind(this, doc, enc, next));
  }
