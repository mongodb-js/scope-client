...
Client.prototype.index = function(ns, name, opts, fn) {
  ...
  var resource = new Resource.Index(this, '/indexes/' + encodeURIComponent(ns), name);
  return !fn ? resource : resource.read(opts, fn);
};
...
Client.prototype.document = function(ns, _id, opts, fn) {
  ...
  var resource = new Resource.Document(this, '/documents/' + encodeURIComponent(ns), _id);
  return !fn ? resource : resource.read(opts, fn);
};
...
Client.prototype.find = function(ns, opts, fn) {
  ...
  return this.read('/collections/' + encodeURIComponent(ns) + '/find', params, fn);
};
...
Client.prototype.count = function(ns, opts, fn) {
  ...
  return this.read('/collections/' + encodeURIComponent(ns) + '/count', params, fn);
};
...
Client.prototype.aggregate = function(ns, pipeline, opts, fn) {
  ...
  return this.read('/collections/' + encodeURIComponent(ns) + '/aggregate', {
...
Client.prototype.sample = function(ns, opts, fn) {
  ...
  if (fn) {
    return this.read('/collections/' + encodeURIComponent(ns) + '/sample', opts, fn);
  }
  opts.ns = ns;
...
function getSampleStream(client, stream_id, options) {
  /**
   ...
   */
  options = EJSON.serialize(options || {});
  debug('creating socket.io stream for', {
    stream_id: stream_id,
    options: options
  });

  var src = ss.createStream({
    objectMode: true
  });

  var deserialize = miss.through({objectMode: true}, function(chunk, enc, cb) {
    cb(null, EJSON.deserialize(chunk));
  });

  ss(client.io).emit(stream_id, src, options);

  return miss.pipe(src, deserialize);
}
...
Client.prototype.createReadStream = function(stream_id, options) {
  debug('create read stream', {stream_id: stream_id, options: options});
  if (this.dead) {
    throw this.dead;
  }
  if (this.closed) {
  ...
  if (this.readable) {
    return getSampleStream(this, stream_id, options);
  }

  debug('not readable yet.  queueing read');
  var client = this;
  var transferred = false;
  var proxy;
  var done;
  proxy = miss.from({objectMode: true}, function(count, fn) {
    done = fn;
