...
module.exports = function(endpoint, instance_id, opts) {
  if (typeof endpoint === 'object') {
    opts = endpoint;
    endpoint = null;
    instance_id = null;
  } else if (typeof instance_id === 'string') {
    var p = Connection.normalizeInstanceId(instance_id).split(':');
    opts.hostname = p[0];
    opts.port = parseInt(p[1], 10);
  } else {
    opts = opts || {};
  }
